...
# no self-attention
class DMPN3_model(base_model):

    def __init__(self, FLAGS, Embedding, sess):
        super(DMPN3_model, self).__init__(FLAGS, Embedding)
        self.now_bacth_data_size = tf.placeholder(tf.int32, shape=[], name="batch_size")
        self.build_model()
        self.init_variables(sess, self.checkpoint_path_dir)
...
# no user-item Collaborative Filtering
class DMPN4_model(base_model):

    def __init__(self, FLAGS, Embedding, sess):
        super(DMPN4_model, self).__init__(FLAGS, Embedding)
        self.now_bacth_data_size = tf.placeholder(tf.int32, shape=[], name="batch_size")
        self.build_model()
        self.init_variables(sess, self.checkpoint_path_dir)
    ...
    def build_model(self):
        ...
        with tf.variable_scope("EnhanceUserPreferenceIntentEncoder"):
            ...
            self.user_enhance_preference_temp_user = gru_net_ins.gru_net(hidden_units=num_units,
                                                         input_data=long_term_intent_temp,
                                                         input_length=self.mask_index)
