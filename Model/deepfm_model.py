...
class DFM_model(base_model):
    ...
    def build_model(self):
        ...
        with tf.variable_scope("LongTermIntentEncoder"):
            ...
            seq_len = attention_output.get_shape().as_list()[1]
        ...
        with tf.variable_scope("EnhanceUserPreferenceIntentEncoder"):
            ...
            deep_net = tf.concat([self.user_enhance_preference_user, attention_pooling], axis=1)
            dence1 = tf.layers.dense(deep_net, embedding_size, activation=tf.nn.relu, use_bias=False)
            self.user_enhance_preference_user = tf.layers.dense(dence1, num_units, activation=tf.nn.relu, use_bias=False)
