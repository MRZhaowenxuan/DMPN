...
class Attention():
    ...
    def vanilla_attention_single(self, queries,
                          keys,
                          keys_length):
        '''
          queries:     [B, H]
        ...
        outputs = tf.matmul(queries, keys_t)  # [B, 1, T]
    ...
    def multihead_attention_single(self,
                                   queries,
                                   keys,
                                   num_units=None,
                                   num_heads=8,
                                   ...
                                   is_training=True,
                                   causality=True,
                                   scope="multihead_attention_single",
                                   reuse=None):
        '''Applies multihead attention.
    ...
    #encoder-decoder construct
    def self_attention_single(self,enc, num_units, num_heads, num_blocks, dropout_rate, is_training, reuse):
        with tf.variable_scope("single_encoder"):
            for i in range(num_blocks):
                with tf.variable_scope("num_blocks_{}".format(i)):
                    ### Multihead Attention
                    enc = self.multihead_attention_single(queries=enc,
                                                       keys=enc,
                                                       num_units=num_units,
                                                       num_heads=num_heads,
                                                       dropout_rate=dropout_rate,
                                                       is_training=is_training,
                                                       scope="self_attention_single",
                                                       causality=True, reuse=None)
                    ### Feed Forward
                    enc = self.feedforward(enc,
                                      num_units=[num_units // 4, num_units],
                                      scope="feed_forward", reuse=reuse)
