...
class SLiRec_model(base_model):
    ...
    def build_model(self):
        ...
        gru = GRU()
        gru_output = gru.bidirectional_gru_net(num_units, self.behavior_embedding_result_dense, input_length=self.seq_length)

        attention_net = Attention()
        # positive_emb = tf.expand_dims(self.positive_embedding_result_dense, 1)
        # attention_output = attention_net.vanilla_attention(gru_output, positive_emb, num_units,
        #                                                        num_heads=1, num_blocks=1, dropout_rate=dropout_rate,is_training=True,
        #                                                        reuse=False,key_length=self.seq_length,
        #                                                        query_length = tf.ones_like(positive_emb[:, 0, 0], dtype=tf.int32))
