...
class mask_data_process():
    ...
    def mask_process_unidirectional(self,type,index,time_window = 24 * 3600 * 35):
        ...
        time_stamp_seq_temp = [self.time_stamp_seq[i] for i in range(0, self.length) if i < temp_index]
        if self.use_action == True:
            action_seq_temp = [self.action_seq[i] for i in range(0, self.length) if i < temp_index]
    ...
    def proc_time_emb(self, time_stamp_seq, mask_time,gap):
        ...
        interval_list = [i + self.offset for i in interval_list]
    ...
    def proc_time(self, time_list, target_time):
        time_interval_list = [time_list[i+1] - time_list[i] for i in range(len(time_list) - 1)]
